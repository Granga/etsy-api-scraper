...
export default class Fields {
    static toTypescript(fields: IField[], type: "parameters" | "interface", interfaceName?: string): string {
        ...
        fields.forEach((field, index) => {
            ts += this.fieldToTS(field, index === fields.length - 1);
        });
    ...
    private static fieldToTS(field: IField, isLast: boolean): string {
        return `${field.field || field.name}${field.required == "N" ? '?' : ''}: ${Fields.mapType(field.type)}${isLast ? '' : ',\n'}`;
    }
